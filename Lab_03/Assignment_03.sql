...
-- Queries
-- Q1: For each level, print the level and the average age of students for that level
SELECT level, AVG(age) AS avg_age
FROM Student
GROUP BY level;

-- Q2: For all levels except JR, print the level and the average age
SELECT level, AVG(age) AS avg_age
FROM Student
WHERE level <> 'JR'
GROUP BY level;

-- Q3: For each faculty member that has taught classes, print name and total number of classes taught
SELECT f.fname, COUNT(c.name) AS total_classes
FROM Faculty f
JOIN Class c ON f.fid = c.fid
GROUP BY f.fname;

-- Q4: Find the highest age of all students
SELECT MAX(age) AS highest_age
FROM Student;

-- Q5: Name and number of students enrolled in Database Systems but not in Operating System Design
SELECT sname, snum
FROM Student s
WHERE s.snum IN (
	SELECT snum
...
-- Q6: Find the average age of students taking a course, only if course has at least 2 students
SELECT e.cname, avg(age) as avg_age
FROM Student s 
JOIN Enrolled e
ON s.snum = e.snum
GROUP BY e.cname
...
-- Q7: Find ids of faculty members teaching more than one course
SELECT fid
FROM Class
GROUP BY fid
HAVING count(DISTINCT name) > 1;

-- Q8: Find ids of students enrolled in more than one course
SELECT snum
FROM Enrolled
GROUP BY snum
HAVING COUNT(DISTINCT cname) > 1;

-- Q9: List students in ascending order of age
SELECT snum, sname, major, level, age
FROM Student
ORDER BY age ASC;

-- Q10: Names and ids of students whose major is some branch of Engineering
SELECT snum, sname
FROM Student
...
-- Q11: Count number of students in each branch of Engineering
SELECT major, COUNT(*) AS num_students
FROM Student
WHERE major LIKE '%Engineering'
GROUP BY major;

-- Q12: Names and ids of faculty who teach Data Structures or Operating System Design
SELECT DISTINCT f.fid, f.fname
FROM Faculty f
JOIN Class c ON f.fid = c.fid
WHERE c.name IN ('Data Structures', 'Operating System Design');

-- Q13: Names and ids of instructors teaching a course and belonging to department id 20
SELECT DISTINCT f.fid, f.fname
FROM Faculty f
JOIN Class c ON f.fid = c.fid
WHERE f.deptid = 20;

-- Q14: Number of courses conducted per room
SELECT room, COUNT(DISTINCT name) AS num_courses
FROM Class
GROUP BY room;
