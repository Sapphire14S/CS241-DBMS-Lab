...
-- !Add column
ALTER TABLE Student
ADD total_credits INT DEFAULT 0;
...
CREATE TRIGGER trg_insert_enrolled
AFTER INSERT ON Enrolled
FOR EACH ROW
BEGIN
    DECLARE credits_val INT;

    SELECT credits INTO credits_val
    FROM Class
    WHERE name = NEW.cname;

    IF NEW.grade IS NOT NULL AND NEW.grade <> 'F' THEN
        UPDATE Student
        SET total_credits = total_credits + credits_val
        WHERE snum = NEW.snum;
    END IF;
END;
//
...
CREATE TRIGGER trg_update_enrolled
...
BEGIN
    ...
    -- Remove old credits
    IF OLD.grade IS NOT NULL AND OLD.grade <> 'F' THEN
        UPDATE Student
        SET total_credits = total_credits - credits_val
        WHERE snum = OLD.snum;
    END IF;
...
CREATE TRIGGER trg_ready_students
AFTER UPDATE ON Student
FOR EACH ROW
BEGIN
    IF NEW.total_credits > 10 THEN
        INSERT IGNORE INTO ready_students VALUES (NEW.snum);
    ELSE
...
-- !for dropping the TRIGGERS 
-- !Can be RECREATED
DROP TRIGGER IF EXISTS trg_insert_enrolled;
DROP TRIGGER IF EXISTS trg_update_enrolled;
DROP TRIGGER IF EXISTS trg_ready_students;
...
-- !Test Queries
-- Check total credits
SELECT snum, total_credits FROM Student;

-- Check ready students
UPDATE Student
SET total_credits = total_credits;
SELECT * FROM ready_students;

-- Test trigger (insert new enrollment)
INSERT INTO Enrolled VALUES (112348546, 'Data Structures', 'A');

-- Test update trigger
UPDATE Enrolled
SET grade = 'A'
WHERE snum = 322654189 AND cname = 'Database Systems';
